...
@click.option("--company", default=None, prompt=True, prompt_required=False, type=str, help="Define the new company")
@click.option("--email", default=None, prompt=True, prompt_required=False, type=str, help="Define the new email")
@click.option("--phone", default=None, prompt=True, prompt_required=False, type=str, help="Define the new phone")
def update(**kwargs) -> FullRequest:
    """Command to update a customer data (fullname, company, email and phone fields)."""
    if all(value is None for key, value in kwargs.items() if key != "customer_id"):
