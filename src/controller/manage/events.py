...
class EventsControllerMixin:
    ...
    def new_event(self, contract_uuid: UUID, name: str) -> None:
        ...
        displayable_event = self.model.add_event(
            contract_uuid, name, employee_id=self.auth.user_id
        )
    ...
    def update_event(self, event_id: int, name: str, start: datetime, end: datetime, attendees: int, location: str,
                     note: str) -> None:
        ...
        fields_name: dict = {"Name": name, "Start": start, "End": end, "Attendees": attendees,
                             "Location": location, "Note": note}
        self.view.display_event(
            displayable_event,
            focus=[name for name, new_value in fields_name if new_value is not None]
        )
