...
@click.option("--username",  prompt=True, prompt_required=True, type=str, help="Specify the employee")
@click.option("--role",  prompt=True, prompt_required=True, default="NONE", type=str, help="Specify the role")
@cli_employee.command(help="Set a new role")
def setrole(username, role):
...
def update(id, username, fullname):
    return Request.EDIT_EMPLOYEE, id, username, fullname
